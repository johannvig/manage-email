...
def verify_email():
    ...
    try:
        with open('', 'r') as csv_file: #put the path of "verify email.csv" file that you can download in the project (ex of a path: "../email/verify email.csv")
...
s = 2
while True:
    with open('', 'r') as csv_file: #put the path of "verify email.csv" file that you can download in the project
        csv_reader = reader(csv_file)
        list_of_rows = list(csv_reader)
        # (list_of_rows)
